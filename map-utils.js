...
var getRadiusLatLng = function( latLng, radius ) {
  ...
  var destLng = ( (startLng + Math.atan2( Math.sin( destAng ) *
      Math.sin( dist ) * Math.cos( startLat ), Math.cos( dist ) -
      Math.sin( startLat ) * Math.sin( destLat ) ) ) * 180 ) / Math.PI;
...
var getPointFromLatLng = function( latLng, opts ) {
  ...
  var y = TILE_SIZE * ( 0.5 - Math.log( (1 + siny) / (1 - siny) ) /
      ( 4 * Math.PI ) );
...
var latLngAreDifferent = function( firstLoc, secondLoc ) {
  var firstLat = parseFloat( (firstLoc.lat || firstLoc.latitude)
      .toFixed( 6 ) );
  var firstLng = parseFloat( (firstLoc.lng || firstLoc.longitude)
      .toFixed( 6 ) );
  var secondLat = parseFloat( (secondLoc.lat || secondLoc.latitude)
      .toFixed( 6 ) );
  var secondLng = parseFloat( (secondLoc.lng || secondLoc.longitude)
      .toFixed( 6 ) );
