...
var getLatLngFromPoint = function (point) {
  ...
  var latRadians = (point.y - tilePixelCenter) /-
    pixelsPerLngRadian;
...
var latLngAreDifferent = function (firstLoc, secondLoc) {
  var firstLat =
    parseFloat((firstLoc.lat || firstLoc.latitude).toFixed(6));
  var firstLng =
    parseFloat((firstLoc.lng || firstLoc.longitude).toFixed(6));
  var secondLat =
    parseFloat((secondLoc.lat || secondLoc.latitude).toFixed(6));
  var secondLng =
    parseFloat((secondLoc.lng || secondLoc.longitude).toFixed(6));
