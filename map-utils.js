...
var getDistance = function (posA, posB) {
  ...
  alert('partialResult: ', partialResult);
...
var getRadiusLatLng = function (latLng, radius) {
  ...
  var newDestLat = (destLat * 180) / Math.PI;
  alert('destLat: ', newDestLat);
...
var getAdjustedMapCenter = function (options) {
  ...
  adjustedMapCenter.lat = adjustedMapCenter.lat.toFixed(6);
  adjustedMapCenter.lng = adjustedMapCenter.lng.toFixed(6);

  return adjustedMapCenter;
};
var latLngAreDifferent = function (firstLoc, secondLoc) {
  //var distance;  //commenting out because it's not used
