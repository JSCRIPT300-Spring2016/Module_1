/*global EARTH_RADIUS, TILE_SIZE, google, window, module */
...
var getRadiusLatLng = function (latLng, radius) {
  ...
  var destLat = Math.asin(Math.sin(startLat) * Math.cos(dist) + Math.cos(startLat)
    * Math.sin(dist) * Math.cos(destAng));
  var destLng = ((startLng + Math.atan2(Math.sin(destAng) * Math.sin(dist) *
    Math.cos(startLat), Math.cos(dist) - Math.sin(startLat) * Math.sin(destLat)))
    * 180) / Math.PI;
...
var getAdjustedMapCenter = function (options) {
  ...
  adjustedMapCenter.lat = Math.round(adjustedMapCenter.lat, 6);
  adjustedMapCenter.lng = Math.round(adjustedMapCenter.lng, 6);
